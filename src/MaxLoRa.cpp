...
MaxLoRa::MaxLoRa(Stream &modemStream, Stream &debugStream, int resetPin, fp_t fp, bool talkative, uint8_t sf, uint8_t fsb)
{
...
void MaxLoRa::onMessage(void (*cb)(const uint8_t *payload, size_t size, port_t port))
{
  messageCallback = cb;
}

bool MaxLoRa::provision(const char *appEui, const char *appKey)
{
...
bool MaxLoRa::join(int8_t retries, uint32_t retryDelay)
{
  int8_t attempts = 0;
...
bool MaxLoRa::join(const char *appEui, const char *appKey, int8_t retries, uint32_t retryDelay)
{
  return provision(appEui, appKey) && join(retries, retryDelay);
}

bool MaxLoRa::personalize(const char *devAddr, const char *nwkSKey, const char *appSKey)
{
  reset(adr);
...
response_t MaxLoRa::sendBytes(const uint8_t *payload, size_t length, port_t port, bool confirm, uint8_t sf)
{
  if (sf != 0)
  {
    setSF(sf);
  ...
  if (!sendPayload(mode, port, (uint8_t *)payload, length))
  {
    ...
    return ERROR_SEND_COMMAND_FAILED;
  }
...
response_t MaxLoRa::poll(port_t port, bool confirm)
{
  uint8_t payload[] = {0x00};
  return sendBytes(payload, 1, port, confirm);
}

void MaxLoRa::sleep(uint32_t mseconds)
{
  if (mseconds < 100)
  {
    return;
...
void MaxLoRa::linkCheck(uint16_t seconds)
{
  clearReadBuffer();
  debugPrint(SENDING);
  sendCommand(MAC_TABLE, MAC_PREFIX, true);
...
void MaxLoRa::debugPrintIndex(uint8_t index, const char *value)
{
  char message[100];
...
void MaxLoRa::debugPrintMessage(uint8_t type, uint8_t index, const char *value)
{
  char message[100];
  switch (type)
  {
...
size_t MaxLoRa::readLine(char *buffer, size_t size, uint8_t attempts)
{
  size_t read = 0;
  while (!read && attempts--)
  {
...
size_t MaxLoRa::readResponse(uint8_t prefixTable, uint8_t index, char *buffer, size_t size)
{
  clearReadBuffer();
...
size_t MaxLoRa::readResponse(uint8_t prefixTable, uint8_t indexTable, uint8_t index, char *buffer, size_t size)
{
  clearReadBuffer();
  sendCommand(prefixTable, 0, true, false);
...
void MaxLoRa::configureEU868()
{
  ...
  sendMacSet(MAC_PWRIDX, PWRIDX_EU868);
}

void MaxLoRa::configureUS915(uint8_t fsb)
{
  uint8_t ch;
  ...
  sendMacSet(MAC_PWRIDX, PWRIDX_US915);
}

void MaxLoRa::configureUS915AB()
{
  sendMacSet(MAC_ADR, "on");
  for (int ch = 0; ch < 72; ch++)
  {
    ...
    {
      sendChSet(MAC_CHANNEL_STATUS, ch, "on");
      sendChSet(MAC_CHANNEL_DRRANGE, ch, "4 4");
    }
    else
    {
      sendChSet(MAC_CHANNEL_STATUS, ch, "off");
...
void MaxLoRa::configureAU915(uint8_t fsb)
{
  uint8_t ch;
  uint8_t chLow = fsb > 0 ? (fsb - 1) * 8 : 0;
  uint8_t chHigh = fsb > 0 ? chLow + 7 : 71;
  ...
  sendMacSet(MAC_PWRIDX, PWRIDX_AU915);
}

void MaxLoRa::configureAS923_925()
{
  ...
  sendMacSet(MAC_PWRIDX, PWRIDX_AS923_925);
}

void MaxLoRa::configureKR920_923()
{
  sendMacSet(MAC_ADR, "off"); // TODO: remove when ADR is implemented for this plan
  ...
  sendMacSet(MAC_PWRIDX, PWRIDX_KR920_923);
}

void MaxLoRa::configureIN865_867()
{
  sendMacSet(MAC_ADR, "off"); // TODO: remove when ADR is implemented for this plan
  ...
  sendMacSet(MAC_PWRIDX, PWRIDX_IN865_867);
}

void MaxLoRa::configureChannels(uint8_t fsb)
{
  switch (fp)
  {
  case FP_EU868:
  ...
  sendMacSet(MAC_RETX, RETX);
}

bool MaxLoRa::setSF(uint8_t sf)
{
  uint8_t dr;
  switch (fp)
  {
...
void MaxLoRa::sendCommand(uint8_t table, uint8_t index, bool appendSpace, bool print)
{
  char command[100];
  switch (table)
  {
...
bool MaxLoRa::sendMacSet(uint8_t index, const char *value)
{
  clearReadBuffer();
  debugPrint(SENDING);
  sendCommand(MAC_TABLE, MAC_PREFIX, true);
...
bool MaxLoRa::sendChSet(uint8_t index, uint8_t channel, const char *value)
{
  clearReadBuffer();
  char ch[5];
  if (channel > 9)
...
bool MaxLoRa::sendJoinSet(uint8_t type)
{
  clearReadBuffer();
  debugPrint(F(SENDING));
  sendCommand(MAC_TABLE, MAC_PREFIX, true);
...
bool MaxLoRa::sendPayload(uint8_t mode, uint8_t port, uint8_t *payload, size_t length)
{
  clearReadBuffer();
  debugPrint(F(SENDING));
  sendCommand(MAC_TABLE, MAC_PREFIX, true);
